...
\end{Schunk}
\begin{figure}[!h]
\includegraphics[width=\textwidth]{Fig1-Osorio-Gonzales-Pinzon}
...
Additionally to the SBML format, the TSV format is the default input of metabolic models for the \CRANpkg{sybil} R package. The TSV format is composed by three text files, following a character-separated (tab by default) value format where each line contains one entry (stoichiometric reaction and associated info). The function \code{writeTSVmod} can writes a metabolic model in a TSV format as follows:
\begin{Schunk}
\begin{Sinput}
...
\end{Sinput}
\end{Schunk}
Metabolic models in TSV format can be readed throug the \code{readTSVmod} function included in the \CRANpkg{sybil} package:
\begin{Schunk}
\begin{Sinput}
